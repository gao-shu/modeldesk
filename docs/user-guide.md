...
3. [模型配置](#3-模型配置)
4. [多模态实测](#4-多模态实测)
5. [Compare（对比）](#5-compare对比)
6. [生成结果](#6-生成结果)
7. [任务进行中与历史](#7-任务进行中与历史)
8. [对外调用（MCP / CLI / Gateway）](#8-对外调用mcp--cli--gateway)
9. [常见问题](#9-常见问题)
...
| 区域 | 入口 | 用途 |
|------|------|------|
| 实测 | 文本 / 图片 / 视频 / 语音 | 单模型 Playground：选模型、写提示词、看结果与历史 |
| 实测 | **Compare** | 同一输入并行对比 2～3 个同模态模型，再 **Use this model** |
...
2. 打开「**启用对象存储**」开关  
3. 页面统一为 **S3 兼容**（七牛 / MinIO / AWS / 各云 S3 网关都填这一套）。未保存过时会预填七牛常见非密钥字段；标签与占位为中英对照  
4. 按表核对后填入 **访问密钥 / Access Key** 与 **密钥 / Secret Key**（只填本机，勿提交 Git）  

| 字段 | 建议值（七牛示例） |
|------|--------|
| 启用对象存储 | 开 |
| 协议 | S3 兼容（`s3`） |
| 存储桶 · Bucket | `images-temp` |
| 区域 · Region | `cn-north-1` |
| 接口地址 · Endpoint | `s3.cn-north-1.qiniucs.com` |
| 公网访问域名 · Public Base URL | `https://img.learncom.cn`（须 HTTPS，末尾不要 `/`） |
| 强制路径风格 · Force path style | 关 |
| 跳过 ACL · Skip ACL | 开（Kodo 常用） |
...
其它 S3 兼容服务（MinIO、AWS、火山/阿里云的 S3 网关等）同样：开开关 → 按中英提示填写 Endpoint 与公网域名 → 保存 → 测试。
...
文本 / 语音步骤相同：选模型 → 填参数 → 运行 → 历史回看。

---

## 5. Compare（对比）

路径：**实测 → Compare**（地址：`/runs/compare`）

用于把 **同一输入** 并行打到 **2～3 个同模态** 模型，再选一个去调 Gateway。

### 步骤

1. 在 **模型配置** 里至少准备 **两个同模态** 模型（都填好 Key）  
2. 打开 **Compare**  
3. 选择模态（文 / 图 / 音 / 视）  
4. 选择 Model A、Model B（可选第三个）  
5. 填写同一提示词与参数 → **Run All**  
6. 查看各列结果：输出 / 产物、latency、TTFT、tokens（上游有则显示）、错误  
7. 在满意的一列点 **Use this model** → 复制 **curl / Python / Java**（指向 `http://127.0.0.1:3300/v1`）
