const base = process.env.API_BASE || 'http://127.0.0.1:9800';
...
ok('page.verify', verify.ok && verifyHtml.includes('测一下你的 Key') && !verifyHtml.includes('最新常用') && verifyHtml.includes('热门模型') && verifyHtml.includes('深测') && verifyHtml.includes('probeSteps') && verifyHtml.includes('resultEvidence') && verifyHtml.includes('sharePanel') && verifyHtml.includes('复制报告地址') && verifyHtml.includes('展开过程明细') && verifyHtml.includes('labelsData') && verifyHtml.includes('未取得信号') && verifyHtml.includes('倾向可信') && verifyHtml.includes('includeSuiteIds') && verifyHtml.includes('includeRawPreview'));
