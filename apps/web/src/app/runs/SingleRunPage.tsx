...
function MediaOutput({
  ...
  if (artifactId && modality === "image") {
      ...
      return (
        <div className="flex h-full w-full items-center justify-center bg-zinc-50/80">
          <button
            ...
            className="flex h-full max-h-full w-full cursor-zoom-in items-center justify-center p-1"
          >
            <img
    ...
    return (
      <div className="grid h-full w-full grid-cols-2 gap-2 overflow-auto bg-zinc-50/80 p-1">
        {allIds.map((id) => (
          <button
            key={id}
            type="button"
  ...
  if (artifactId && modality === "video") {
    return <VideoPlayer src={artifactUrl(artifactId)} />;
  }
  return (
...
export function SingleRunPage({ modality }: { modality: Modality }) {
  ...
  async function startRun() {
    ...
    const params: Record<string, unknown> = { ...runParams };
    setViewedRun(null);
    setBootError(null);
  ...
  const [promptHeight, setPromptHeight] = useState(() =>
    modality === "text" ? 320 : 200,
  );
  const [paramsOpen, setParamsOpen] = useState(() => compactLeft);

  function startPromptResize(e: ReactPointerEvent<HTMLDivElement>) {
    e.preventDefault();
    const startY = e.clientY;
    const startH = promptHeight;
    const minH = compactLeft ? 96 : 140;
    const onMove = (ev: PointerEvent) => {
      setPromptHeight(
        Math.min(640, Math.max(minH, startH + (ev.clientY - startY))),
      );
    };
    const onUp = () => {
      ...
      {models.length === 0 && allModels.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:items-stretch">
            <section className="flex max-h-[min(42rem,calc(100vh-8.5rem))] flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white">
            ...
            <section className="flex flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-100 px-3.5 py-2.5">
              ...
              <div
                ...
                style={{ minHeight: Math.max(280, promptHeight + 120) }}
              >
              ...
              {(() => {
                ...
                return (
                  <div className="mt-auto flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-t border-zinc-100 bg-zinc-50/70 px-3.5 py-2">
