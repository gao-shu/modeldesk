...
type VideoPlayerProps = {
  src: string;
  /** Extra classes on the <video> element (e.g. rounded). */
  className?: string;
 ...
 * The video viewport is height-capped by the wrapper so tall media letterboxes.
 */
export function VideoPlayer({
  src,
  className = "rounded-md object-contain",
  wrapperClassName = "flex h-full min-h-0 w-full flex-col gap-2 bg-zinc-50/80 p-2",
  preload = "metadata",
}: VideoPlayerProps) {
    ...
    <div className={wrapperClassName}>
      <div className="relative min-h-0 w-full flex-1 overflow-hidden">
        <video
          ...
          preload={preload}
          src={src}
          className={`absolute inset-0 m-auto max-h-full max-w-full ${className}`}
          onVolumeChange={(e) => {
