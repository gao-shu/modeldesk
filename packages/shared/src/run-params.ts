...
import { applyFormatParamAliases, getApiFormat } from "./api-formats";
...
export type RunParamField = {
  ...
  endKey?: string;
  /**
   * For image_pair (optional): multi reference list key (e.g. Seedance
   * `reference_images` with role=reference_image). When set, UI adds「多参考」模式。
   */
  listKey?: string;
   ...
   */
  allowPair?: boolean;
  /** For image_pair: label of the single-image mode (default「首帧」; Grok may use「首帧图」). */
  refModeLabel?: string;
};
...
export function videoSettingsFromParams(
  ...
  // Grok 用顶层 `duration`（1-15 秒，官方默认 8），其它格式用 `duration_sec`
  const durationRaw = Number(params.duration_sec ?? params.duration);
  const grokVideo = apiFormatId === "video.grok";
...
export function resolveRunParamsForFormat(
  ...
  const result: Record<string, unknown> = {};
  const ov = applyFormatParamAliases(formatId, overrides, formatOverride);
  for (const f of format.fields) {
    ...
    if (f.type === "image_pair") {
      ...
      // Multi-ref list (Seedance / Grok R2V): must not drop listKey.
      const listKey = f.listKey?.trim();
      if (listKey) {
        const listRaw = ov[listKey] ?? modelDefaults[listKey];
        if (Array.isArray(listRaw)) {
          const items = listRaw
            .filter((x): x is string => typeof x === "string")
            .map((x) => x.trim())
            .filter(Boolean);
          if (items.length > 0) result[listKey] = items;
        } else if (
          typeof listRaw === "string" &&
          listRaw.trim() &&
          listRaw.trim() !== "[]"
        ) {
          result[listKey] = listRaw.trim();
        }
      }
    }
  }
