      ...
      {pendingDelete ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="md-delete-model-title"
            className="w-full max-w-sm rounded-lg border border-zinc-200 bg-white p-4 shadow-lg"
            onClick={(e) => e.stopPropagation()}
          >
            <h2
              id="md-delete-model-title"
              className="text-sm font-semibold text-zinc-900"
            >
              是否确定删除
            </h2>
            <p className="mt-2 text-sm text-zinc-600">
              确定删除模型「{pendingDelete.name}」？此操作无法恢复。
            </p>
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                className="h-9 rounded-md border border-zinc-300 bg-white px-3.5 text-sm text-zinc-700 hover:bg-zinc-50"
                onClick={() => setPendingDelete(null)}
              >
                取消
              </button>
              <button
                type="button"
                className="h-9 rounded-md bg-red-600 px-3.5 text-sm text-white hover:bg-red-700"
                onClick={() => {
