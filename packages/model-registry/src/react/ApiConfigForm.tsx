      ...
      >
        ...
        {modalityUsesApiFormatPicker(form.modality) ? (
          <>
            ...
            {activeFormat?.hint ? (
              <p className="md-hint">{activeFormat.hint}</p>
            ) : null}
          </>
        ) : (
          <>
        ...
        {showBaseUrlMode ? (
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex h-8 overflow-hidden rounded-md border border-zinc-300 text-sm">
              <button
                type="button"
            ...
            {baseUrlMode === "simple" && form.baseUrl.trim() ? (
              <span className="md-hint min-w-0 break-all">
                请求时自动补全为{" "}
